...
public interface RentalBookRepository extends JpaRepository<RentalBook, Long> {
	...
	@Query(value = "select nvl(count(REN_BORROW),0) from RENTAL where "
			+ "TO_CHAR(REN_BORROW, 'YYYY/MM/DD')=:date",
			nativeQuery = true)
	...
	@Query(value = "select NVL(COUNT(REN_BORROW), 0) from RENTAL where BOOK_NUM in " + 
			...
			"where CF_NAME=:catgo)) and REN_BORROW>=TO_DATE(:fDay,'YYYY-MM-DD') and REN_BORROW<=TO_DATE(:eDay, 'YYYY-MM-DD')",
			nativeQuery = true)
	...
	@Query(value = "select BOOK_NUM from " + 
			...
			"from RENTAL where REN_BORROW>=TO_DATE(:fDay, 'YYYY-MM-DD') and REN_BORROW<=TO_DATE(:eDay, 'YYYY-MM-DD') " + 
	...
	@Query(value = "select COUNT(*) from RENTAL where REN_BORROW=TO_DATE(:date, 'YYYY-MM-DD') " + 
	...
	@Query(value = "select * from (select ROWNUM rnum, aa.* from "
			+ "(select * from RENTAL where MEM_NUM=:mem_num order by REN_STAT asc, BOOK_NUM desc) aa)"
			+ " where rnum>:startRow and rnum<=:endRow",
			nativeQuery = true)
	List<RentalBook> selectPerPageMem(@Param("startRow") int startRow, @Param("endRow") int endRow, @Param("mem_num") long mem_num); //회원이 빌린 책들
	
	@Query(value = "select count(*) from RENTAL where MEM_NUM=:mem_num",
			nativeQuery = true)
	Long getRentalBookMem(@Param("mem_num") long mem_num); //회원이 빌린 책 수
}
