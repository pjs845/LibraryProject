...
public interface BookRequestRepository extends JpaRepository<BookRequestDAO, Long> {
	...
	@Query(value = "select * from (select ROWNUM rnum, aa.* from " + 
			"(select * from BOOK_REQUEST where MEM_NUM=:mem_num order by BR_STAT asc, BR_NUM desc) aa) " + 
			"where rnum>:startRow and rnum<=:endRow",
			nativeQuery = true)
	List<BookRequestDAO> selectPerPageMem(@Param("startRow") int startRow, @Param("endRow") int endRow, @Param("mem_num") long mem_num); //회원이 요청한 책들
	
	@Query(value = "select count(*) from BOOK_REQUEST where MEM_NUM=:mem_num",
			nativeQuery = true)
	Long getRequestBookMem(@Param("mem_num") long mem_num); //회원이 요청한 책 수
}
