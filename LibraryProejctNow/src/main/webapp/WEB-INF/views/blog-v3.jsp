...
<body>
                    ...
                    <div class="container">
                        <div class="d-md-flex justify-content-between align-items-center py-4">
                            ...
                            <nav class="woocommerce-breadcrumb font-size-2">
                                <a class="btn btn-blue btn-sm-wide transition-3d-hover" href="/admin/book/add">도서추가</a>
                            </nav>
                        </div>
                    </div>
                </div>
            ...
            <div class="container">
                                ...
                                <script>
									jQuery(function($) {
										...
										$("#admin_search_button").click(function(e) {					// 메인 페이지 검색어 입력박스 옆 전송버튼 클릭시
											...
											$(location).attr('href', "/admin/book/search?catgo="+catgo+"&keyword="+keyword+"&cp=1&ps=10&searchModeStr=T");
											return false;
										});
									});
								</script>
                            ...
                            <div id="Categories" class="widget p-4 border">
                                <div id="Heading22">
                                    <a class="d-flex align-items-center justify-content-between collapse-link text-dark" href="#"
                                        ...
                                        <h3 class="title mb-0 font-weight-medium font-size-3">장르</h3>
                                ...
                                <div id="Collapse22" class="mt-4 collapse"
                                    ...
                                    data-parent="#SidebarAccordion">
                                    	<a class="form-group d-flex align-items-center justify-content-between font-size-2 text-lh-md mb-3" href="/admin/book/search?catgo=0&cp=1&ps=10&searchModeStr=T">
                                            <span class="text-dark">전체</span>
                                            <small class="text-gray-600"></small>
                                        </a>
                                    	<c:forEach items="${listResult.clsList}" var="cls" varStatus="list">
                                        <a class="form-group d-flex align-items-center justify-content-between font-size-2 text-lh-md mb-3" href="/admin/book/search?catgo=${list.count}&cp=1&ps=10&searchModeStr=T">
                                            <span class="text-dark">${cls}</span>
                                            <c:if test="${not empty listResult.clsCount[list.index]}">
                                            	<small class="text-gray-600">${listResult.clsCount[list.index]}</small>
                                            </c:if>
                                            <c:if test="${empty listResult.clsCount[list.index]}">
                                            	<small class="text-gray-600">0</small>
                                            </c:if>                                            
                                        </a>
                                        </c:forEach>
                                </div>
                    ...
                    <c:forEach items="${listResult.bookListResult.bookList}" var="book">
                        <div class="d-md-flex align-items-center mb-5">
                            ...
                            <div class="ml-md-8">
                                ...
                                <p class="font-size-2 text-secondary-gray-700">장르 : ${listResult.clsList[book.book_code-1]}</p>
                            ...
                            &nbsp;                            
                            <a class="btn btn-primary-red btn-sm-wide transition-3d-hover" href="javascript:deleteBook(${book.book_num});"">삭제</a>
                        </div>  
                        <hr>
                      </c:forEach>             
                    </div>
                ...
                <script>
	                function clickPage(cp, ps){
						...
						if(search!=""){ //검색했을 때
							...
							$(location).attr('href', "/admin/book/search?catgo="+catgo+"&cp="+cp+"&ps="+ps+"&keyword="+keyword+"&searchModeStr="+search);
						}else{ //검색 안했을 때							
							$(location).attr('href', "/admin/book/search?cp="+cp+"&ps="+ps);							
						}
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination pagination__custom justify-content-md-center flex-nowrap flex-md-wrap overflow-auto overflow-md-visble">
                        	<c:if test="${listResult.bookListResult.startpaging != 1 }">
                        			<li class="flex-shrink-0 flex-md-shrink-1 page-item"><a class="page-link" href="javascript:void(0);" onclick="clickPage(${listResult.bookListResult.startpaging-1}, 10);">이전</a></li>
							</c:if>
						    <c:forEach begin="${listResult.bookListResult.startpaging }" end="${listResult.bookListResult.endpaging }" var="i">
						        <a href="list.do?ps=${ps}&cp=${i}">
						   			<c:choose>
						   			    <c:when test="${i==listResult.bookListResult.cp}">
						    ...
						    <c:if test="${listResult.bookListResult.endpaging != listResult.bookListResult.totalPageCount}">						
									<li class="flex-shrink-0 flex-md-shrink-1 page-item"><a class="page-link" href="javascript:void(0);" onclick="clickPage(${listResult.bookListResult.endpaging+1}, 10);">다음</a></li>
							</c:if>
                        </ul>
                </nav>
            </div>
    ...
    <!-- ====== END MAIN CONTENT ===== -->
	<script>
		function deleteBook(book_num){			
			var result = confirm("정말로 삭제하시겠습니까?");
			if(result){
				$(location).attr('href', "/admin/book/del?book_num="+book_num);
			}else{
