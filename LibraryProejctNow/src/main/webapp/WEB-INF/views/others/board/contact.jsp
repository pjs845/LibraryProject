...
<body>
    ...
    <main id="content">
        <div class="py-3 py-lg-7">
            <h6 class="font-weight-medium font-size-7 text-center my-1">자유 게시판</h6>
        </div>
        <div id="mainWrapper">
           ...
           <!-- 게시판 페이징 영역 -->
            <li>
                ...
                <c:choose>
	           		<c:when test="${keyword != null}">
	           			<c:choose>
			           		<c:when test="${start != 1}">
			                	<div><a href="read.do?catgo=${catgo}&keyword=${keyword}&cp=${start-1}">◀</a></div>
			                </c:when>
		                </c:choose>	
	                </c:when>  
	                <c:when test="${keyword == null}">
	           			<c:choose>
			           		<c:when test="${start != 1}">
			                	<div><a href="read.do?cp=${start-1}">◀</a></div>
			                </c:when>
		                </c:choose>	
	                </c:when>  
	            </c:choose>    	            
	                 ...
	                 <c:choose>
	              	   	<c:when test="${keyword != null}">
	              	   <div><a href="read.do?catgo=${catgo}&keyword=${keyword}&cp=${i}">${i}</a></div>
	              	   </c:when>
	              	   <c:otherwise>
	               	 <div><a href="read.do?cp=${i}">${i}</a></div>
	               	 </c:otherwise>
	               	</c:choose>
					</c:forEach>  
                ...
                <c:choose>
	           		<c:when test="${keyword != null}">
	           			<c:choose>
			           		<c:when test="${start != 1}">
			                	<div><a href="read.do?catgo=${catgo}&keyword=${keyword}&cp=${end+1}">▶</a></div>
			                </c:when>
		                </c:choose>	
	                </c:when>  
	                <c:when test="${keyword == null}">
	           			<c:choose>
			           		<c:when test="${start != 1}">
			                	<div><a href="read.do?cp=${end+1}">▶</a></div>
			                </c:when>
		                </c:choose>	
	                </c:when>  
	            </c:choose>  
			...
			<li>
				<div>
					 ...
					 <button type="submit" class="btn btn-wide btn-dark text-white rounded-0 transition-3d-hover height-60" onclick="location.href='/write.do'">글쓰기</button>
				</div>
			</li>
			<br>
            ...
            <form action="read.do">
	            <li id='catgo'>
	                <div>
	                    <select name="catgo" id='selSearchOption' >        
	                        <option value='title'>제목</option>
	                        <option value='content'>내용</option>
	                        <option value='name'>작성자</option>
	                    </select>
	                    <input id='txtKeyWord' name="keyword"/>
	                    <button type="submit" style="background:black; color:white; width:135px; height:40px; ">검색</button>
	                </div>
	             </li>
            </form>
            <br>
