        ...
        <div class="mb-5 mb-lg-8 pb-xl-1">
                ...
                <div class="page-header border-bottom">
                    <div class="container">
                        <div class="d-md-flex justify-content-between align-items-center py-4">
                            <h1 class="page-title font-size-3 font-weight-medium m-0 text-lh-lg">도서신청 리스트</h1>
            ...
            <div class="container">
                <div class="row mb-lg-5 mb-xl-0">
                                ...
                                <script>
									jQuery(function($) {
										...
										$("#admin_search_button").click(function(e) {					// 메인 페이지 검색어 입력박스 옆 전송버튼 클릭시
											...
											$(location).attr('href', "/admin/request/manage?catgo="+catgo+"&keyword="+keyword+"&cp=1&ps=10&searchModeStr=T");
											return false;
										});
									});
								</script>
                            ...
                            <div id="Categories" class="widget p-4 border">
                                <div id="Heading22">
                                    <a class="d-flex align-items-center justify-content-between collapse-link text-dark" href="#"
                                        ...
                                        <h3 class="title mb-0 font-weight-medium font-size-3">도서신청상황</h3>
                                ...
                                <div id="Collapse22" class="mt-4 collapse"
                                    ...
                                    data-parent="#SidebarAccordion">
                                    	<a class="form-group d-flex align-items-center justify-content-between font-size-2 text-lh-md mb-3" href="/admin/request/manage">
                                            <span class="text-dark">전체</span>
                                            <small class="text-gray-600"></small>
                                        </a>
                                        <a class="form-group d-flex align-items-center justify-content-between font-size-2 text-lh-md mb-3" href="/admin/request/manage?catgo=wait&cp=1&ps=10&searchModeStr=T">
                                            <span class="text-dark">신청대기중</span>
                                            <small class="text-gray-600"></small>
                                        </a>
                                        <a class="form-group d-flex align-items-center justify-content-between font-size-2 text-lh-md mb-3" href="/admin/request/manage?catgo=ok&cp=1&ps=10&searchModeStr=T">
                                            <span class="text-dark">신청승인</span>
                                            <small class="text-gray-600"></small>
                                        </a>                                                    	
                                </div>
                            </div>
                    ...
                    <c:forEach items="${listResult.brlr.items}" var="list">
                        <div class="d-md-flex align-items-center mb-5">
                            <span class="d-block mb-3 mb-md-0">
                                <img class="img-fluid" src="/img/books_img.png" alt="Image-Description" width=150px; height=150px;>
                            </span>
                            <div class="ml-md-8">
                                <div class="mb-2">
                                	<c:choose>
                                		<c:when test="${list.br_stat==0}">
		                                	<span class="font-size-2">상태 : <span style="color:red;">신청대기중</span></span>
                                		</c:when>
                                		<c:otherwise>
                                			<span class="font-size-2">상태 : <span style="color:green;">신청승인</span></span>
                                		</c:otherwise>
                                	</c:choose>
                                ...
                                <h6 class="font-size-7 crop-text-2 font-weight-medium text-lh-1dot4" style="width:400px;">
                                    <span>제목 : ${list.br_title}</span>
                                </h6>
                                <p class="font-size-2 text-secondary-gray-700">저자 : ${list.br_writer}</p>
                                <p class="font-size-2 text-secondary-gray-700">ISBN : ${list.br_isbn}</p>     
                                <p class="font-size-2 text-secondary-gray-700">신청회원번호 : ${list.mem_num}</p>                              
                            </div>
                            
                            <c:choose>
	                         	<c:when test="${list.br_stat==0}">
	                            	<a class="btn btn-primary btn-sm-wide transition-3d-hover" href="javascript:request_ok(${list.br_isbn});">승인</a>
                            		&nbsp;
                            		<a class="btn btn-primary-red btn-sm-wide transition-3d-hover" href="javascript:request_deny(${list.br_isbn}, ${list.mem_num});">거절</a>
	                         	</c:when>
	                         	<c:otherwise>
	                         		<button type="button" class="btn btn-primary btn-sm-wide transition-3d-hover" onclick="modalPopup(${list.br_isbn});" data-toggle="modal" data-target="#myModal">등록</button>
	                         		&nbsp;
                            		<a class="btn btn-primary-red btn-sm-wide transition-3d-hover" href="javascript:request_deny(${list.br_isbn}, ${list.mem_num});">삭제</a>
	                         	</c:otherwise>
	                         </c:choose>                                                                                 
                        </div>                        
                        <hr>
				  ...
				  <div class="modal-dialog" role="document">
				    <div class="modal-content">
				      <div class="modal-header">
				        ...
				        <h4 class="modal-title" id="myModalLabel">도서추가</h4>
				      </div>
				      <div class="modal-body" id="modal_body">				        
				      </div>
				      <div class="modal-footer">
				        <button type="button" class="btn btn-punch" data-dismiss="modal">닫기</button>
				      </div>
				    </div>
				  </div></div>
                ...
                <script>
                	function request_ok(isbn){ //도서신청 수락
                		var result = confirm("신청을 수락하시겠습니까?");
            			if(result){
            				$(location).attr('href', "/admin/request/ok?isbn="+isbn);
            			}else{            				
            			}
                	}                	
                	function request_deny(isbn){ //도서신청 거절
                		var result = confirm("거절하시겠습니까?");
            			if(result){
            				$(location).attr('href', "/admin/request/del?isbn="+isbn);
            			}else{            				
            			}
                	}
                	
                	function request_enter(isbn){ //도서신청 등록
                		var result = confirm("도서를 등록하시겠습니까?");
            			if(result){
            				$(location).attr('href', "/admin/request/enter?isbn="+isbn);
            			}else{            				
            			}
                	}
	                ...
	                function clickPage(cp, ps){
						...
						if(search!=""){ //검색했을 때
							...
							$(location).attr('href', "/admin/request/manage?catgo="+catgo+"&cp="+cp+"&ps="+ps+"&keyword="+keyword+"&searchModeStr="+search);
						}else{ //검색 안했을 때							
							$(location).attr('href', "/admin/request/manage?cp="+cp+"&ps="+ps);							
						}						
					}
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination pagination__custom justify-content-md-center flex-nowrap flex-md-wrap overflow-auto overflow-md-visble">
                        	<c:if test="${listResult.brlr.startpaging != 1}">
                        			<li class="flex-shrink-0 flex-md-shrink-1 page-item"><a class="page-link" href="javascript:void(0);" onclick="clickPage(${listResult.brlr.startpaging-1}, 10)">이전</a></li>
							</c:if>
						    <c:forEach begin="${listResult.brlr.startpaging}" end="${listResult.brlr.endpaging}" var="i">
						        <a href="list.do?ps=${ps}&cp=${i}">
						   			<c:choose>
						   			    <c:when test="${i==listResult.brlr.cp}">
						    ...
						    <c:if test="${listResult.brlr.endpaging != listResult.brlr.totalPageCount}">						
									<li class="flex-shrink-0 flex-md-shrink-1 page-item"><a class="page-link" href="javascript:void(0);" onclick="clickPage(${listResult.brlr.endpaging+1}, 10)">다음</a></li>
							</c:if>
                        </ul>
                </nav>
            </div>
